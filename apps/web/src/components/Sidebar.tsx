...
export default function Sidebar() {
  ...
  const clearDraftThreadsForProject = useComposerDraftStore(
    (store) => store.clearDraftThreadsForProject,
  );
  ...
  const handleNewThread = useCallback(
    (
      ...
      return createOrReuseProjectDraftThread(
        {
  ...
  const focusMostRecentThreadForProject = useCallback(
    (projectId: ProjectId) => {
      const latestThreadId = findMostRecentThreadIdForProject(threads, projectId);
  ...
  const addProjectFromPath = useCallback(
    async (rawCwd: string) => {
      ...
      try {
        const result = await ensureProjectExists(projects, cwd);
        ...
        await handleNewThread(result.projectId).catch(() => undefined);
      } catch (error) {
        const description =
      ...
      await deleteThreadWithCleanup(
        {
          ...
          navigateAfterDelete: ({ deletedThreadId, fallbackThreadId }) => {
